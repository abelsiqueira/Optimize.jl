function test_blackbox()
  @testset "Blackbox" begin
    ...
    @testset "Bounded quadratic" begin
      ...
      nlp = SimpleNLPModel(f, zeros(2), lvar=zeros(2), uvar=0.5*ones(2))
    ...
    @testset "Linearly constrained quadratic" begin
      f(x) = (x[1] - 1)^2 + 4 * (x[2] - 2)^2
      c(x) = x[1] + x[2] - 1.0
      nlp = SimpleNLPModel(f, zeros(2), c=c, lcon=[-Inf], ucon=[0.0])
      x, fx = blackbox(nlp, verbose=false, max_f=1_000_000, tol=1e-12)
    ...
    @testset "Linearly constrained quadratic with bounds" begin
      f(x) = (x[1] - 1)^2 + 4 * (x[2] - 2)^2
      c(x) = x[1] + x[2] - 1.0
      nlp = SimpleNLPModel(f, zeros(2), lvar=zeros(2), ucon=ones(2), c=c,
                           lcon=[-Inf], ucon=[0.0])
      x, fx = blackbox(nlp, verbose=false, max_f=1_000_000, tol=1e-12)
    ...
    @testset "Narrow" begin
      ...
      c(x) = x[2] - x[1]
      ϵ = 1e-4
      nlp = SimpleNLPModel(f, ones(2), c=c, lcon=[-ϵ], ucon=[ϵ])
      x, fx = blackbox(nlp, verbose=false, max_f=1_000_000, tol=1e-12)
      @test norm(x) < 1e-6
    end
